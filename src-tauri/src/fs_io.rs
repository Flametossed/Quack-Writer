...
/// Font formats accepted by the "Import font" picker.
const FONT_EXTENSIONS: [&str; 4] = ["ttf", "otf", "woff", "woff2"];

/// Validate a generated font UID: ASCII alphanumeric plus dashes only, so the
/// string can't escape the dedicated app-data `fonts/` folder through `..`/`/`.
...
/// Open a native file picker filtered to font files and return the chosen path.
#[tauri::command]
pub async fn pick_font_file(window: Window) -> Result<Option<String>, String> {
    let path = window
        .dialog()
        .file()
...
/// Copy a picked font file into the app-data `fonts/` folder under a UID so it
/// survives relaunches. The UID is generated by the frontend and validated
/// here to keep the target path confined to that folder.
#[tauri::command]
pub async fn import_font_file(
    ...
    let source = PathBuf::from(&picked_path);
    if !source.is_file() {
    ...
    let filename = source
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| "Invalid font path".to_string())?
        .to_string();
    let ext = source
        .extension()
        .and_then(|e| e.to_str())
        .map(|e| e.to_ascii_lowercase())
        .filter(|e| FONT_EXTENSIONS.contains(&e.as_str()))
        .ok_or_else(|| "Unsupported font type".to_string())?;

    let fonts_dir = app_handle
        .path()
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?
        .join("fonts");
    fs::create_dir_all(&fonts_dir)
        .map_err(|e| format!("Failed to create fonts folder: {e}"))?;

    let saved_path = fonts_dir
        .join(format!("{uid}.{ext}"))
        .to_string_lossy()
        .to_string();
    fs::copy(&source, &saved_path).map_err(|e| format!("Failed to copy font: {e}"))?;
    Ok(ImportedFont {
        filename,
        saved_path,
    })
...
/// Read a stored font file and hand it back as raw bytes for the webview's
/// FontFace API. ArrayBuffer is sent straight through Tauri's IPC, no base64.
#[tauri::command]
pub fn read_font_bytes(path: String) -> Result<Response, String> {
    let buf = fs::read(&path).map_err(|e| format!("Failed to read font: {e}"))?;
    Ok(Response::new(buf))
}

/// Best-effort delete of a stored font file. A missing file is treated as
/// success so a stale registry entry can still be cleaned up.
#[tauri::command]
pub fn delete_font_file(path: String) -> Result<(), String> {
    match fs::remove_file(&path) {
        Ok(()) => Ok(()),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(e) => Err(format!("Failed to delete font: {e}")),
    }
}
